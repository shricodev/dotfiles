  ...
  config = function()
    require('gitsigns').setup {
      ...
      on_attach = function(bufnr)
        ...
        -- Actions
        -- map('n', '<leader>hs', gitsigns.stage_hunk, { desc = keymap_namer(pname, 'stage hunk') })
        -- map('v', '<leader>hs', function()
        --   gitsigns.stage_hunk { vim.fn.line '.', vim.fn.line 'v' }
        -- end, { desc = keymap_namer(pname, 'stage hunk') })
        ...
        -- map('n', '<leader>hu', gitsigns.stage_hunk, { desc = keymap_namer(pname, 'undo stage hunk') })

        map('n', '<leader>hv', gitsigns.preview_hunk, { desc = keymap_namer(pname, 'preview hunk') })

        -- map('n', '<leader>hb', function()
        --   gitsigns.blame_line { full = true }
        -- end, { desc = keymap_namer(pname, 'blame line') })
        --
        -- map('n', '<leader>hd', gitsigns.diffthis, { desc = keymap_namer(pname, 'diff this') })
        -- map('n', '<leader>hD', function()
        --   gitsigns.diffthis '~'
        -- end, { desc = keymap_namer(pname, 'diff this ~') })
      end,
    }
  end,
}
